...
const pickQuote = () =>{
    randIndex = Math.floor(Math.random() * quoteBank.length)

    document.getElementById("text").innerText = quoteBank[randIndex].text;

if (!quoteBank[randIndex].author){
    quoteBank[randIndex].author = "Anonymous"
}

    document.getElementById("author").innerText = quoteBank[randIndex].author;


    let adjustedText = quoteBank[randIndex].text
//     adjustedText = adjustedText.replace(/\W/g, "%20")
// console.log(adjustedText)
    let adjustedAuthor = quoteBank[randIndex].author;

//attempting to remove white spaces and other special characters

//     adjustedAuthor = adjustedAuthor.replace(/\W/g, "%20")
// console.log(adjustedAuthor)


    let tweetLink = "https://twitter.com/intent/tweet?text=";

    tweetLink += adjustedText;
    
    tweetLink += "-" + adjustedAuthor;
